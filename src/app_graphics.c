...
static int init_marker_draw_buf_rgb565(void)
{
	...
	marker_draw_buf_desc.height = MARKER_BUF_DIM;
	marker_draw_buf_desc.buf_size = MARKER_BUF_DIM * MARKER_BUF_DIM * display_bits_per_pixel / 8;
...
static int draw_marker_mono(uint16_t x0, uint16_t y0)
{
	...
	LOG_DBG("draw_marker_mono write to display dev x0:%d y0:%d w:%d h:%d", 0,
